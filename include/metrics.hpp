...
struct OperationStats {
    ...
    void record(uint64_t probes) {
        total_probes += probes;
        ++operations;
        if (probes > worst_probes) worst_probes = probes;
    }
...
inline size_t compute_max_cluster(const uint64_t* keys, size_t table_size, uint64_t empty_sentinel) {
    ...
    size_t wrap = 0;
    for (size_t i = 0; i < table_size && keys[i] != empty_sentinel; ++i) ++wrap;
    size_t tail = 0;
    for (size_t i = table_size; i > 0; --i) {
        if (keys[i - 1] != empty_sentinel) ++tail;
        else break;
    }
    if (wrap > 0 && tail > 0) {
        size_t merged = wrap + tail;
        if (merged > max_run) max_run = merged;
    }
