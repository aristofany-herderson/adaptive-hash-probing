...
private:
    enum class SearchOutcome { Found, EmptyGap, BlockFull };
    ...
    size_t insert_in_block(size_t start_cell, uint64_t key, uint64_t& probes) {
        ...
        for (size_t round = 0; round < block_count; ++round) {
            ...
            if (block_loads_[current_block] >= cells) continue;
            ...
            for (size_t i = 0; i < cells; ++i) {
                ...
                ++probes;
                if (table_[pos] == kEmptyKey) return pos;
            }
        }
        return capacity_;
    }
    ...
    SearchOutcome search_from_cell(size_t start_cell, uint64_t key, uint64_t& probes) const {
        ...
        for (size_t i = 0; i < cells; ++i) {
            ...
            if (table_[pos] == key) return SearchOutcome::Found;
        }

        if (saw_empty) return SearchOutcome::EmptyGap;
        if (block_loads_[bid] < cells) return SearchOutcome::EmptyGap;
...
private:
    size_t terminal_empty(size_t start, uint64_t& probes) const {
        ...
        while (true) {
            ++probes;
            if (table_[idx] == kEmptyKey) return idx;
            idx = (idx + 1) % capacity_;
        }
    }
